import * as dotenv from 'dotenv'
import fetch from "node-fetch";
import { createApi } from "unsplash-js";
import { userAttemptsModel } from "../models/user_attempts.js";
import { transporter } from "./nodemailer.js";
dotenv.config()

function checkArray(arr1, arr2, sequence) {
    if (arr1.length != arr2.length) return false;
    var gflag = false;
    if (sequence){
        for(let i=0; i<arr1.length; i++) {
            if (arr1[i] != arr2[i]) {
                gflag = false;
                break;
            }
            else gflag = true;
        }
    } 
    else {    
        for(let i=0; i<arr1.length; i++) {
        var flag = false;
        for(let j=0; j<arr2.length; j++) {
            if (arr1[i] === arr2[j]) {flag = true;}
        }
        gflag = flag;
        }
    }
    return gflag;
}

// New function to validate pattern categories
function validatePatternCategories(pattern) {
    // Validate pattern structure
    if (!Array.isArray(pattern) || pattern.length !== 5) {
        return {
            isValid: false,
            errors: ['Pattern must contain exactly 5 images']
        }
    }

    // Extract categories
    const categories = pattern.map(item => item.category)
    
    // Check unique categories
    const uniqueCategories = new Set(categories)
    
    // Validate category count
    if (uniqueCategories.size < 1 || uniqueCategories.size > 5) {
        return {
            isValid: false,
            errors: ['Categories must be between 1 and 5 unique categories']
        }
    }

    // Optional: Add validation for category names if needed
    // You can add specific category validation logic here
    
    return {
        isValid: true,
        categories: Array.from(uniqueCategories)
    }
}

const unsplash = createApi({
    accessKey: 'txkfz6Y7Iz8CsisA04s4KZj92dUsYpXoCg_AuGFqfRo',
    fetch: fetch,
});

function shuffleArray(array) {
    for (var i = array.length - 1; i > 0; i--) {
        var j = Math.floor(Math.random() * (i + 1));
        var temp = array[i];
        array[i] = array[j];
        array[j] = temp;
    }
}

async function sendEmail(email) {
    const currentUser = await userAttemptsModel.findOne({email: email})
    const mailOptions = {
        from: "graphicalpassauth@gmail.com",
        to: email,
        subject: "GPA | Account Blocked",
        html: `<div>
                <p>Your account has been blocked for multiple attempts of login with invalid credentials.</p>
                <p>Click the link below to unblock:</p>
                <a href='${process.env.URL}/api/verify?email=${email}&token=${currentUser.token}'>Unblock</a>
               </div>`
    }
    console.log("Sending email to " + email)
    transporter.sendMail(mailOptions, function(err, info) {
        if (err) console.log(err)
        else console.log("Email sent to " + email)
    })
}

// Additional helper function to get category names for user-friendly display
function getNameByNumber(number) {
    const names = ['First', 'Second', 'Third', 'Fourth', 'Fifth']
    return names[number] || 'Unknown'
}

export { 
    checkArray, 
    unsplash, 
    shuffleArray, 
    sendEmail, 
    validatePatternCategories,
    getNameByNumber
}